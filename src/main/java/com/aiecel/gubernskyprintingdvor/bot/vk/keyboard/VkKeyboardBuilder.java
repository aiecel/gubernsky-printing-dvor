package com.aiecel.gubernskyprintingdvor.bot.vk.keyboard;

import com.vk.api.sdk.objects.messages.*;

import java.util.ArrayList;
import java.util.List;

public class VkKeyboardBuilder implements KeyboardBuilder<KeyboardButton, Keyboard> {
    public static final int MAX_ROWS = 10;
    public static final int MAX_COLUMNS = 5;
    public static final int MAX_BUTTONS = 40;

    private final KeyboardButton[][] buttons = new KeyboardButton[MAX_COLUMNS][MAX_ROWS];
    private int buttonCount;

    @Override
    public KeyboardBuilder<KeyboardButton, Keyboard> add(KeyboardButton button) {
        //find first empty row
        for (int row = 0; row < buttons[0].length; row++) {
            if (isRowEmpty(row)) {
                //add button to this row
                return add(button, row, 0);
            }
        }

        //if there's no empty rows then find first non empty place
        for (int row = 0; row < buttons[0].length; row++) {
            for (int column = 0; column < buttons.length; column++) {
                if (buttons[column][row] == null) {
                    //add button to this row
                    return add(button, row, column);
                }
            }
        }

        return this;
    }

    @Override
    public KeyboardBuilder<KeyboardButton, Keyboard> add(KeyboardButton button, int row, int column) {
        if (buttonCount < MAX_BUTTONS) {
            buttons[column][row] = button;
            buttonCount++;
        }
        return this;
    }

    @Override
    public Keyboard build() {
        Keyboard keyboard = new Keyboard();
        List<List<KeyboardButton>> allButtons = new ArrayList<>();

        for (int row = 0; row < buttons[0].length; row++) {
            if (!isRowEmpty(row)) {
                List<KeyboardButton> rowButtons = new ArrayList<>();
                for (KeyboardButton[] button : buttons) {
                    if (button[row] != null) {
                        rowButtons.add(button[row]);
                    }
                }
                allButtons.add(rowButtons);
            }
        }

        keyboard.setButtons(allButtons);
        keyboard.setOneTime(true);
        return keyboard;
    }

    private boolean isRowEmpty(int row) {
        for (KeyboardButton[] button : buttons) {
            if (button[row] != null) return false;
        }
        return true;
    }
}
